import 'dart:ui';

import 'package:hexcolor/hexcolor.dart';

class AppColors {
  static Color primaryBackground = HexColor('#F9F9F9');
  static const Color primaryText = Color.fromARGB(255, 0, 0, 0);
  static Color firstText = HexColor("#2F2D2C");
  static Color secondaryText = HexColor("#A9A9A9");
  static Color thirdText = HexColor("#DDDDDD");
  static Color fouthText = HexColor("#B7B7B7");
  static Color fifthText = HexColor("#2F4B4E");
  static Color subText = HexColor("#9B9B9B");
  static Color divider = HexColor("#EAEAEA");
  static Color sizeBackground = HexColor("#FFF5EE");
  static Color sizeBorder = HexColor("#DEDEDE");
  static Color botttomNavBorder = HexColor("#F1F1F1");
  static Color botttomNavShadow = HexColor("#E4E4E4");
  static Color disableNavIcon = HexColor("#8D8D8D");
  static Color hintText = HexColor("#989898");
  static Color backgroundTextField = HexColor("#313131");

  static Color primaryElement = HexColor("#C67C4E");
}
